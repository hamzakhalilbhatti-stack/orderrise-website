...
function makeScene(type){
  ...
  else if(type==="about"){const core=new THREE.Mesh(new THREE.IcosahedronGeometry(1.1,3),new THREE.MeshStandardMaterial({color:C.green,emissive:C.green,emissiveIntensity:1.2,metalness:.4,roughness:.2}));core.position.y=1.7;root.add(core);for(let i=0;i<3;i++){const r=new THREE.Mesh(new THREE.TorusGeometry(1.8+i*.55,.025,8,80),new THREE.MeshBasicMaterial({color:i===2?C.greenLight:C.green,transparent:true,opacity:.55-i*.1,toneMapped:false}));r.rotation.set(i*.7,Math.PI/2+i*.45,0);r.position.y=1.7;root.add(r);}}
  else if(type==="contact"){const p=phone();p.position.set(-2.4,2.2,0);p.rotation.y=.3;root.add(p);["BOOK DEMO","WHATSAPP","CALENDAR"].forEach((t,i)=>{const s=sprite(t,i===0?"#68ff9b":"#f7f8fa",i===0?"rgba(37,211,102,.14)":"rgba(13,16,21,.88)",.62);s.position.set(1.6,2.8-i*1.15,0);root.add(s);});}
  else {
    const solution=type.replace("solution-","");
    const label=sprite(solution.replaceAll("-"," ").toUpperCase(),"#68ff9b","rgba(37,211,102,.14)",.75);label.position.set(0,4,0);root.add(label);
    if(solution==="pizza-restaurants"){const oven=box(3,2.2,1.8,0x453329,.28);oven.position.set(0,1.1,-1);root.add(oven);for(let i=0;i<4;i++){const pizza=cyl(.65,.65,.1,0xeabf6d);pizza.position.set(-2.2+i*1.45,.55,1.3);root.add(pizza);}}
    else if(solution==="cafes"){const counter=box(5,.85,1.3,0x3b2c24,.15);counter.position.y=.55;root.add(counter);for(let i=0;i<4;i++){const cup=cyl(.28,.24,.55,0xf7f8fa);cup.position.set(-1.8+i*1.2,1.15,0);root.add(cup);}}
    else if(solution==="cloud-kitchens"){for(let i=0;i<3;i++){const p=panel(`ORDER SCREEN ${i+1}`,[C.green,C.amber,C.blue][i]);p.scale.setScalar(.55);p.position.set(-3+i*3,2.2,-1.2);root.add(p);}root.add(building(0,1.4,C.surface2));}
    else if(solution==="burger-restaurants"){for(let i=0;i<5;i++){const b=burger();b.position.set(-3+i*1.5,.2,1);root.add(b);}const grill=box(4,.7,1.5,0x2a3037,.12);grill.position.set(0,.5,-1);root.add(grill);}
    else if(solution==="bakeries"){for(let i=0;i<4;i++){const cake=cyl(.55,.6,.6,[0xf2a5b9,0xf3d06a,0xb9e8c0,0xd8b3ff][i]);cake.position.set(-2.3+i*1.55,.45,.9);root.add(cake);}const shelf=building(0,-1.5,0x332d32);root.add(shelf);}
    else if(solution==="takeaway"){const shelf=box(5,2.5,.6,0x2d333a,.12);shelf.position.set(0,1.25,-1.3);root.add(shelf);for(let i=0;i<6;i++){const bag=box(.65,.9,.4,C.amber,.08);bag.position.set(-2+i*.8,.6+(i%2)*1.05,-.9);root.add(bag);}}
...
}catch(error){console.error(error);document.querySelector(`[data-fallback-for="${canvas.id}"]`)?.classList.add("show");}
